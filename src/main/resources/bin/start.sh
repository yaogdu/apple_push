...
DEPLOY_DIR=`pwd`
#配置文件目录
CONF_DIR=$DEPLOY_DIR/conf:$DEPLOY_DIR/conf/mappers:$DEPLOY_DIR/conf/spring
#jar包目录
...
JAVA_MEM_OPTS=" -server -Xms5g -Xmx5g -Xmn1900m -Xss256k -XX:PermSize=128m -XX:MaxPermSize=512m
...
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_MEM_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS com.demai.main.Main > $STDOUT_FILE 2>&1 &
